...
const out = path.join(root, 'output/v8');
...
const opening = await selectComposition({...opt,id:'OpeningV8-1080',inputProps:{}});
if(mode==='stills'){
  const frames=[0,35,120,240,360,448,500,620,750,820,980,1120,1300,1450,1660,1840,1949];
  ...
  await renderMedia({...opt,composition:opening,inputProps:{},frameRange:[0,opening.durationInFrames-1],muted:true,codec:'h264',crf:19,imageFormat:'jpeg',jpegQuality:92,concurrency:4,outputLocation:path.join(out,'V8片头总览预览1080p.mp4')});
  console.log('OPENING COMPLETE');
}else if(mode==='full'){
  const comp=await selectComposition({...opt,id:'ChapterV8-1080',inputProps:props});
  await renderMedia({...opt,composition:comp,inputProps:props,frameRange:[0,comp.durationInFrames-1],muted:true,codec:'h264',crf:20,imageFormat:'jpeg',jpegQuality:92,concurrency:6,outputLocation:path.join(out,'第一章_看见与选择_V8_无声1080p.mp4')});
