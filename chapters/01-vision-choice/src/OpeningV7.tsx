...
export function OpeningV7({overview=false}:{overview?:boolean}){
 ...
 return <AbsoluteFill style={{background:'#050b11'}}>
  <div style={{position:'absolute',width:1920,height:1080,transform:`scale(${width/1920})`,transformOrigin:'0 0',fontFamily:'NotoSansSC',overflow:'hidden'}}>
   ...
   <div style={{position:'absolute',inset:0,clipPath:handoff>0?`inset(0 ${handoff*100}% 0 0)`:undefined}}>
    ...
    <div style={{position:'absolute',left:mix(overview?1710:1380,1608,move),top:mix(overview?80:280,70,move),color:'#d9c6a3'}}>
     <svg width={mix(overview?64:260,40,move)} height={mix(overview?75.6:307.2,48,move)} viewBox={mark.viewBox} style={{display:'block'}}>
     ...
     <div style={{position:'absolute',left:mix(overview?32:130,50,move),top:mix(overview?88:334,6,move),transform:`translateX(${-50*(1-move)}%) translateY(${(1-name)*7}px)`,fontFamily:'NotoSerifSC',fontSize:mix(overview?24:52,30,move),fontWeight:600,letterSpacing:mix(2,1,move),whiteSpace:'nowrap',opacity:name}}>rainbow鱼</div>
    </div>
...
export function LegacyFilmV7({audio=true,overview=false}:{audio?:boolean;overview?:boolean}){
 return <AbsoluteFill>
  <Sequence durationInFrames={config.duration}><OpeningV7 overview={overview}/></Sequence>
