...
export function BrandV7({f,closing=false}:{f:number;closing?:boolean}){
...
export function FilmV7({audio=true}:{audio?:boolean}){
 ...
 return <AbsoluteFill style={{background:'#050b11',fontFamily:'NotoSansSC'}}>
  {f<config.opening?<Sequence durationInFrames={config.opening}><OpeningV7 overview/></Sequence>:<>
   ...
   {closing&&<>
    ...
    <BrandV7 f={f-config.outroFrom} closing/>
   </>}
   {cap&&!closing&&<div style={{position:'absolute',left:115,right:115,top:946,textAlign:'center',fontSize:46,lineHeight:1.4,color:'#fff2dc',fontWeight:600,textShadow:'0 2px 7px #000,0 0 20px #000'}}>{cap.text}</div>}
  ...
  {audio&&<Audio src={staticFile('audio/v7/master.wav')}/>}
 </AbsoluteFill>;
}
