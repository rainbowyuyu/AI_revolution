...
import config from './timeline-v8.json';
...
export function FilmV8({audio=true}:{audio?:boolean}){
 ...
 return <AbsoluteFill style={{background:'#050b11',fontFamily:'NotoSansSC'}}>
  {f<config.opening?<Sequence durationInFrames={config.opening}><OpeningV8/></Sequence>:<>
   ...
   {f>=config.opening&&f<config.opening+60&&<AbsoluteFill style={{opacity:1-ease((f-config.opening)/60),pointerEvents:'none'}}><OffthreadVideo src={staticFile('media/bridge-01.mp4')} muted startFrom={149} style={{width:1920,height:1080,objectFit:'cover'}}/><AbsoluteFill style={{background:'linear-gradient(90deg,#040c14a8,#07141d55 58%,#06121d88),linear-gradient(0deg,#040b13b8,transparent 28%,transparent 78%,#040b1370)'}}/></AbsoluteFill>}
