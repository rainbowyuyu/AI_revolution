...
import config from './opening-v9.json';
...
function HandoffMotion({f}:{f:number}) {
  const from=config.handoffFrom;
  const bridgeIn=ease((f-(from-90))/72);
  const bridgeOut=ease((f-(from+18))/78);
  const taskIn=ease((f-(from-24))/100);
  const titleIn=ease((f-from)/55);
  const taskFrame=Math.min(149,Math.max(0,f-(from-24)));
  return <>
    <AbsoluteFill style={{opacity:bridgeIn*(1-bridgeOut),pointerEvents:'none'}}>
      <OffthreadVideo src={staticFile('media/bridge-01.mp4')} muted startFrom={Math.min(149,Math.max(0,f-(from-90)))} style={{width:1920,height:1080,objectFit:'cover'}}/>
      <AbsoluteFill style={{background:'linear-gradient(90deg,#040b14b8,#07151d55 58%,#06121d90),radial-gradient(ellipse at 72% 48%,#79cbd01c,transparent 55%)'}}/>
    </AbsoluteFill>
    <AbsoluteFill style={{opacity:taskIn,pointerEvents:'none'}}>
      <OffthreadVideo src={staticFile('media/hero-vision.mp4')} muted startFrom={taskFrame} style={{width:1920,height:1080,objectFit:'cover'}}/>
      <AbsoluteFill style={{background:'linear-gradient(90deg,#040c14d8 0%,#07141d92 52%,#06121d55),linear-gradient(0deg,#040b13c4,transparent 35%,transparent 78%,#040b1366)'}}/>
      <div style={{position:'absolute',left:120,top:168,opacity:titleIn,transform:`translateY(${(1-titleIn)*18}px)`}}>
        <div style={{fontSize:26,letterSpacing:5,color:'#afc2c3'}}>第一章 · 看见与选择</div>
        <div style={{marginTop:22,fontFamily:'NotoSerifSC',fontSize:76,color:'#f2e8d2',fontWeight:650,letterSpacing:2}}>任务一：认出没见过的图片</div>
        <div style={{marginTop:28,width:520,height:2,background:'linear-gradient(90deg,#d6bd85,transparent)',transformOrigin:'left',transform:`scaleX(${titleIn})`}}/>
      </div>
      <div style={{position:'absolute',left:122,top:890,opacity:titleIn*.9,fontSize:24,color:'#b8c8c8',letterSpacing:1.2}}>把一张新图片交给计算机，看它怎样找到线索。</div>
    </AbsoluteFill>
  </>;
}

export function OpeningV9(){
  ...
  const handoff=ease((f-config.handoffFrom)/55);
